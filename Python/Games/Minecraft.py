...
# player = FirstPersonController(model='cube', z=-10, color=color.blue, origin_y=-.5, speed=8, scale= 0.7, collider='box',position=(0,3,0))           
    ...
    def explode(self):
        for entity in scene.entities:
            if distance(entity.position, self.position) <= 3 and isinstance(entity, Voxel):
                destroy(entity)
            # if isinstance(entity, TNT) and distance(player.position, entity.position) < 1:
...
# voxel = Voxel(position=(5, 4, 5))
# voxel = Voxel(position=(5, 5, 5))

# voxel = Voxel(position=(5, 4, 10))
# voxel = Voxel(position=(5, 5, 10))

# voxel = Voxel(position=(10, 4, 5))
# voxel = Voxel(position=(10, 5, 5))

# voxel = Voxel(position=(10, 4, 10))
# voxel = Voxel(position=(10, 5, 10))
